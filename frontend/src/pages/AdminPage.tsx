...
function UserManagement() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "http://localhost:5000"}`}/api/users`, {
        headers: {
          "x-auth-token": token || "",
        },
      });
  ...
  const handleSendInvitation = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "http://localhost:5000"}`}/api/users/invite`, {
        method: "POST",
        headers: {
  ...
  const handleRoleChange = async (userId: number, newRole: string) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "http://localhost:5000"}`}/api/users/${userId}/role`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          "x-auth-token": token || "",
  ...
  const handleDeleteUser = async (userId: number) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${import.meta.env.VITE_API_URL || `${import.meta.env.VITE_API_URL || "http://localhost:5000"}`}/api/users/${userId}`, {
        method: "DELETE",
        headers: {
          "x-auth-token": token || "",
        },
